...
src_prepare() {
	...
	epatch "${FILESDIR}/${MY_PN}-2.28.3-fix-codegen-location.patch"

	# Do not build tests if unneeded, bug #226345
	epatch "${FILESDIR}/${MY_PN}-2.28.3-make_check.patch"

	# Support installation for multiple Python versions, upstream bug #648292
	epatch "${FILESDIR}/${MY_PN}-2.28.3-support_multiple_python_versions.patch"

	# Disable tests that fail
	epatch "${FILESDIR}/${MY_P}-disable-failing-tests.patch"

	# Disable introspection tests when we build with --disable-introspection
	epatch "${FILESDIR}/${MY_P}-tests-no-introspection.patch"

	# Fix warning spam
	epatch "${FILESDIR}/${MY_P}-set_qdata.patch"
	epatch "${FILESDIR}/${MY_P}-gio-types-2.32.patch"

	# Fix glib-2.36 compatibility, bug #486602
	epatch "${FILESDIR}/${MY_P}-glib-2.36-class_init.patch"
